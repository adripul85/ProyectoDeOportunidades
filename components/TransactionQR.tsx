...
export const TransactionQR: React.FC<TransactionQRProps> = ({ value, label }) => {
    return (
        <div className="flex flex-col items-center w-full">
            <div className="bg-white p-4 rounded-xl border-2 border-dashed border-light-200 mb-4 w-full flex justify-center">
                <QRCode
                    value={value}
                    size={180}
            ...
            {label && (
                <div className="text-center">
                    ...
                    <p className="text-2xl font-black text-dark-800 font-mono tracking-tighter">{label}</p>
                </div>
            )}
        </div>
    );
